 ...
 * jsPDF + jspdf-autotable + pdf-lib (merge con folleto IPID del producto)
 */

// jsPDF, autoTable y pdf-lib se cargan dinámicamente dentro de generateQuotePdf()
// para evitar que vendor-pdf (~306 KiB) se incluya en el bundle inicial de la home.
import type jsPDF from "jspdf";
import { CAMPAIGN_AUTONOMOS_IDS, getAutonomosDiscountTier } from "@/data/campanaSalud2026";
...
export interface QuoteData {
  ...
  totalPuntos:           number;
  puntosXAseg:           number;
  /** Meses gratis de campaña (particulares): número de meses, "descuento25" o null si no aplica. */
  mesesGratis:           number | "descuento25" | null;
  hayNulos:              boolean;
}
...
/* ═══════════════════════════════════════════════════════════════
   FUNCIÓN PRINCIPAL
═══════════════════════════════════════════════════════════════ */
export async function generateQuotePdf(quote: QuoteData, cliente: ClienteInfo): Promise<void> {
  // Carga diferida: evita que vendor-pdf cargue en páginas que no lo necesitan
  ...
  /* ════════════════════════════════════════════════════════════
     ...
     Campaña Salud 2026 (vigencia 1 jun - 31 dic 2026). Cifras únicas
     en @/data/campanaSalud2026.
     · GO: sin sección.
     · Negocios NIF / Pymes Total: descuento en prima por tramos (azul).
     · Plena Total / Vital Total con ≥3 aseg: 25% de descuento (magenta).
     · Resto de particulares (incl. Seniors): meses gratis + puntos (magenta).
  ════════════════════════════════════════════════════════════ */
  {
    const nAsegPub      = quote.preciosPorPersona.length;
    const es25pct       = quote.mesesGratis === "descuento25";
    ...
    if (!sinPromo && cliente.includePuntos !== false) {
      ...
      if (esAutonomos) {
        ...
        const pct = getAutonomosDiscountTier(quote.id, nAsegPub) * 100;
        ...
        doc.text(`${pct}% DE DESCUENTO EN LA PRIMA`, ML + 8, y + 13.5);
        doc.setFont("helvetica", "normal"); doc.setFontSize(7); doc.setTextColor(29, 78, 216);
        doc.text(
          quote.id === "negocios-nif"
            ? "5% (1-3 aseg.) · 10% (4 o mas aseg.) sobre la prima"
            : "5% (1-3 aseg.) · 15% (4 o mas aseg., 7,5% en 1a renovacion) sobre la prima",
          RX, y + 13.5, { align: "right" }
        );
      } else {
        // Franja magenta para meses gratis / 25% descuento (particulares, incl. Seniors)
        ...
        if (es25pct) {
          ...
          const meses = typeof quote.mesesGratis === "number" ? quote.mesesGratis : 0;
          doc.setFont("helvetica", "bold"); doc.setFontSize(9); doc.setTextColor(...MAGENTA);
          doc.text(
            meses > 0 ? `${meses} ${meses === 1 ? "MES GRATIS" : "MESES GRATIS"}` : "PUNTOS DE CAMPANA",
            ML + 8, y + 13.5,
          );
  ...
  /* ── Mapa producto → fichero IPID (/public/product-pdfs/) ── */
  const IPID_MAP: Record<string, string> = {
  ...
  if (!ipidPath) {
    // Sin folleto IPID para este producto → descarga directa
    doc.save(fileName);
    return;
  }

  /* ── Fusionar presupuesto + folleto IPID con pdf-lib ── */
  try {
    // 1. Bytes del presupuesto generado por jsPDF
    const quoteBytes = doc.output("arraybuffer");

    // 2. Descargar folleto IPID del producto
