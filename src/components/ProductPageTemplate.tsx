    ...
    <div className="container mx-auto px-4 py-10 lg:py-14 relative z-10">
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-10 items-center">
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="lg:pl-14 xl:pl-24">
          ...
          <div className={`flex items-center gap-4 mb-8 flex-wrap`}>
            ...
            {data.useWhatsAppCta && (
              <a
                ...
                className="flex items-center gap-2 px-6 py-3 rounded-lg font-bold text-sm cursor-pointer btn-cta-whatsapp"
          ...
          {/* CTA buttons (non-WhatsApp pages) */}
          {!data.useWhatsAppCta && (
            /* Mobile only: CTA buttons (tarificador is below on mobile) */
            <div className="flex flex-wrap gap-3 lg:hidden">
              {data.customTarificador ? (
                <button
                  onClick={onMobileCalc}
                ...
                <CalcButton productSlug={data.productSlug} className="px-6 py-3 rounded-lg font-bold text-sm cursor-pointer btn-cta-white" style={{ backgroundColor: "#fff", color: "#003087", borderRadius: "7px" }}>
                  Calcular mi precio →
                </CalcButton>
              )}
              <button onClick={openPhonePopup} className="px-6 py-3 rounded-lg font-bold text-sm border cursor-pointer btn-cta-ghost" style={{ borderColor: "rgba(255,255,255,0.4)", color: "#fff", borderRadius: "7px" }}>
                Te llamamos ahora
              </button>
            </div>
          )}
